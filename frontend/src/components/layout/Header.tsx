...
import { RightTopHorizontalBar } from '@/components/layout/bars';
...
export function Header({ onToggleSidebar }: HeaderProps) {
  ...
  const rightBarItems = [
    // Notifications
    <button key="notifications" className="p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors">
    ...
    isLoading ? (
      <div key="loading" className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
      ...
      <div key="auth-buttons" className="flex items-center space-x-2">
  ...
  return (
    <>
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-slate-700 sticky top-0 z-40">
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            {/* Right Side Actions - Hide on large screens when using RightTopHorizontalBar */}
      ...
      {/* Right Top Horizontal Bar - Show on large screens */}
      <div className="hidden lg:block">
        <RightTopHorizontalBar items={rightBarItems} className="mt-4" />
      </div>
    </>
  );
}
