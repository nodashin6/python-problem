...
class TestProblemApplicationService:
    ...
    async def test_get_published_problems_success(
        self, problem_app_service, mock_problem_repo
    ):
        """公開済み問題取得成功のテスト"""
        # モックの設定
        problems = [
    ...
    @pytest.mark.asyncio
    async def test_get_problems_by_book_id(
        self, problem_app_service, mock_problem_repo
    ):
        """問題集ID指定問題取得のテスト"""
        book_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_get_problems_by_difficulty(
        self, problem_app_service, mock_problem_repo
    ):
        """難易度指定問題取得のテスト"""
        problems = [
        ...
        # テスト実行
        result = await problem_app_service.get_problems_by_difficulty(
...
class TestJudgeCaseApplicationService:
    ...
    @pytest.mark.asyncio
    async def test_get_judge_cases_by_problem_id(
        self, judge_case_app_service, mock_judge_case_repo
    ):
        """問題ID指定ジャッジケース取得のテスト"""
        problem_id = uuid4()
        judge_cases = [
    ...
    @pytest.mark.asyncio
    async def test_get_sample_cases_only(
        self, judge_case_app_service, mock_judge_case_repo
    ):
        """サンプルケースのみ取得のテスト"""
        problem_id = uuid4()
        ...
        mock_judge_case_repo.find_sample_cases_by_problem_id.assert_called_once_with(
            problem_id
        )

    @pytest.mark.asyncio
    async def test_get_judge_case_by_id(
        self, judge_case_app_service, mock_judge_case_repo
    ):
        """ID指定ジャッジケース取得のテスト"""
        case_id = uuid4()
...
class TestUserProblemStatusApplicationService:
    ...
    async def test_get_user_problem_status(self, status_app_service, mock_status_repo):
        ...
        mock_status_repo.find_by_user_and_problem.assert_called_once_with(
            user_id, problem_id
        )
    ...
    @pytest.mark.asyncio
    async def test_update_user_problem_status_success(
        self, status_app_service, mock_status_repo
    ):
        """ユーザー問題状態更新成功のテスト"""
        user_id = uuid4()
        problem_id = uuid4()
