...
class JudgeCaseLoader:
    ...
    def __iter__(self) -> Iterator[models.JudgeCase]:
        """テストケースのイテレータを返します"""
        judge_case_names = self.load_judge_case_names()
        for judge_case_name in judge_case_names:
            try:
                yield self.load_judge_case(judge_case_name)
            except FileNotFoundError as e:
                logging.error(
                    f"テストケース {judge_case_name} の読み込みに失敗: {str(e)}"
                )

    def load_judge_case_names(self) -> List[str]:
        """テストケース名のリストを読み込みます"""
        # パス: problems/問題集/問題ID/testcase.yaml
        yaml_path = (
            PROBLEM_DIR / self.problem_set / f"{self.problem.id}" / "testcase.yaml"
        )

        try:
            with open(yaml_path, "r") as f:
                judge_case_name = yaml.safe_load(f)

            # 数値のテストケース名を適切にフォーマット
            for i in range(len(judge_case_name)):
                if isinstance(judge_case_name[i], int):
                    judge_case_name[i] = f"{judge_case_name[i]:02d}"

            return judge_case_name
    ...
    def get_input_path(self, judge_case_name: str) -> Path:
        """入力ファイルのパスを返します"""
        ...
        return (
            ...
            / "testcases"
            / "in"
            / f"{judge_case_name}.txt"
        )

    def get_output_path(self, judge_case_name: str) -> Path:
        """出力ファイルのパスを返します"""
    ...
    def load_judge_case(self, judge_case_name: str) -> models.JudgeCase:
        """テストケース名からテストケースを読み込みます"""
        in_path = self.get_input_path(judge_case_name)
        out_path = self.get_output_path(judge_case_name)
        ...
        return models.JudgeCase(
            id=judge_case_name,
            name=judge_case_name,
            problem=self.problem.model_dump(),
            stdin=models.Stdin(id="", name=judge_case_name, content=stdin),
            stdout=models.Stdout(id="", name=judge_case_name, content=stdout),
        )
...
class JudgeProcessResult:
    ...
    def __init__(self, is_ac: bool, metadata: models.JudgeCaseResultMetadata):
        self.is_ac = is_ac
...
class CodeExecutor:
    ...
    def execute(
    ...
    ) -> tuple[str, str, int, int, Optional[int]]:
        """
        コードを実行して結果を返す
        ...
        try:
            ...
            def set_limits():
                # CPUタイムの制限
                resource.setrlimit(
                    resource.RLIMIT_CPU, (self.TIME_LIMIT, self.TIME_LIMIT)
                )
                # メモリ使用量の制限
                resource.setrlimit(
...
class JudgeUserCode:
    ...
    def judge_case(self, code: str, JudgeCase: models.JudgeCase) -> JudgeProcessResult:
        """
        ...
        Returns:
            JudgeProcessResult: ジャッジ結果
        """
        stdin = JudgeCase.stdin.content
        expected_stdout = JudgeCase.stdout.content

        try:
            # コードを実行
            stdout, stderr, execution_time, return_code, memory_used = (
                self.code_executor.execute(code, stdin)
            )
            ...
            if return_code != 0:
                ...
                return JudgeProcessResult(False, metadata)
            ...
            return JudgeProcessResult(is_ac, metadata)
    ...
    def determine_status(self, result: JudgeProcessResult) -> str:
        """ジャッジ結果のステータスを決定する"""
        if result.is_ac:
            return "AC"
...
class AppUseCase:
    ...
    def read_problem(self) -> models.Problem:
        ...
        problem_path = (
            PROBLEM_DIR / self.problem_set / f"{self.problem.id}" / "problem" / "ja.md"
        )
        with open(problem_path, "r") as f:
            problem_md = f.read()

        return models.Problem(id=self.problem.id, markdown=problem_md)
...
class ReadProblemListUseCase:
    ...
    def __call__(self) -> list[models.Problem]:
        ...
        for row in rows:
            ...
            problems.append(
                models.Problem(id=row["id"], title=row["title"], level=level)
            )
        return problems
