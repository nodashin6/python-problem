...
class ProblemApplicationService:
    ...
    async def get_published_problems(
        self, book_id: Optional[UUID] = None
    ) -> List[Problem]:
        """公開されている問題一覧を取得"""
        try:
            if book_id:
    ...
    async def get_problem_with_judge_cases(
        ...
        try:
            ...
            judge_cases = await self.judge_case_repository.find_by_problem_id(
                problem_id
            )
            ...
            logger.info(
                f"Problem with {len(judge_cases)} judge cases retrieved: {problem.title}"
            )
            return result
...
class JudgeCaseApplicationService:
    ...
    async def get_judge_cases_by_problem(self, problem_id: UUID) -> List[JudgeCase]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(judge_cases)} judge cases for problem {problem_id}"
            )
            return judge_cases
        except Exception as e:
    ...
    async def get_public_judge_cases(self, problem_id: UUID) -> List[JudgeCase]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(public_cases)} public judge cases for problem {problem_id}"
            )
            return public_cases
        except Exception as e:
            logger.error(
                f"Error getting public judge cases for problem {problem_id}: {e}"
            )
            raise
...
class UserProblemStatusApplicationService:
    ...
    def __init__(
        self, user_problem_status_repository: UserProblemStatusRepositoryInterface
    ):
    ...
    async def get_user_status(
        ...
        try:
            status = await self.user_problem_status_repository.find_by_user_and_problem(
                user_id, problem_id
            )
            if status:
            ...
            logger.error(
                f"Error getting user status for {user_id}, problem {problem_id}: {e}"
            )
            raise
    ...
    async def get_user_all_statuses(self, user_id: str) -> List[UserProblemStatus]:
        """ユーザーの全問題解決状況を取得"""
        try:
            statuses = await self.user_problem_status_repository.find_by_user_id(
                user_id
            )
            logger.info(
                f"Retrieved {len(statuses)} problem statuses for user {user_id}"
            )
            return statuses
        except Exception as e:
    ...
    async def update_user_status(
        ...
        try:
            # 既存のステータスを取得
            existing_status = (
                await self.user_problem_status_repository.find_by_user_and_problem(
                    user_id, problem_id
                )
            )
            ...
            if existing_status:
                ...
                updated_status = await self.user_problem_status_repository.update(
                    existing_status
                )
                ...
                created_status = await self.user_problem_status_repository.create(
                    new_status
                )
