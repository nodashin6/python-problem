...
class User(Entity):
    ...
    def __init__(self, **data):
        ...
        if not hasattr(self, "_events") or not self._events:
            self.add_event(
                UserRegistered(user_id=self.id, email=self.email, role=self.role)
            )
...
class Problem(Entity):
    ...
    def __init__(self, **data):
        ...
        self.add_event(
            ProblemCreated(
                problem_id=self.id, title=self.title, author_id=self.author_id
            )
        )
    ...
    def publish(self) -> None:
        ...
        if self.status == ProblemStatus.DRAFT:
            ...
            self.updated_at = datetime.utcnow()
            self.add_event(
                ProblemPublished(problem_id=self.id, published_by=self.author_id)
            )
...
class LegacyJudgeCase(BaseModel):
    """ジャッジケース定義 (Legacy)"""
