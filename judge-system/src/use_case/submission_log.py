...
class SubmissionLogger:
    ...
    def save_submission(self, judge_response: models.JudgeResponse, problem_set: str) -> models.SubmissionLog:
        """
        提出内容と結果をログとして保存する
        """
        # 提出IDを生成（UUIDv4）
        ...
        submission_dir = SUBMISSIONS_DIR / problem_set / problem_id / now.strftime("%Y/%m/%d")
    ...
    def get_submission_logs(self, problem_id: str, problem_set: str, limit: int = 10) -> list[models.SubmissionLog]:
        """
        指定した問題の提出ログを取得（新しい順）
        """
        # 提出ログディレクトリ
    ...
    def get_submission_details(self, submission_id: str) -> tuple[models.SubmissionLog, models.SubmissionResult] | None:
        """
        提出IDから提出ログと結果の詳細を取得
        """
        # 提出ログを含むファイルを検索
