...
from src.shared.auth import PasswordManager, JWTManager
...
class TestUserDomainService:
    ...
    @pytest.fixture
    def user_service(
        self, mock_user_repo, mock_password_manager, mock_token_manager, mock_event_bus
    ):
    ...
    async def test_change_password_wrong_old_password(
        ...
        # テスト実行
        result = await user_service.change_password(
            user_id, "wrongoldpassword", "newpassword"
        )
    ...
    async def test_update_user_profile(self, user_service, mock_user_repo):
        ...
        new_profile = UserProfile(
            display_name="New Name", bio="New bio", github_username="newusername"
        )
