...
class ProblemController:
    ...
    async def get_problem(
        self, problem_id: UUID, user_id: Optional[str] = None
    ) -> ProblemDetailResponse:
        """問題詳細取得"""
        try:
            problem_data = await self.problem_service.get_problem_with_judge_cases(
                problem_id
            )
            if not problem_data:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="問題が見つかりません"
                )
            ...
            user_status = None
            if user_id:
                status = await self.user_status_service.get_user_status(
                    user_id, problem_id
                )
                if status:
                    user_status = {
...
class JudgeCaseController:
    ...
    async def get_public_judge_cases(self, problem_id: UUID) -> List[JudgeCaseResponse]:
        """公開ジャッジケース取得"""
        try:
            judge_cases = await self.judge_case_service.get_public_judge_cases(
                problem_id
            )
            return [
                JudgeCaseResponse(
