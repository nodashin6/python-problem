...
ADD
    CONSTRAINT submissions_status_check CHECK (
        status IN ('pending', 'judging', 'completed', 'error')
    );
...
ALTER TABLE
    public.judge_processes
ADD
    CONSTRAINT judge_processes_status_check CHECK (
        status IN (
            'pending',
            'running',
            'succeeded',
...
ALTER TABLE
    public.judge_case_results
ADD
    CONSTRAINT judge_case_results_status_check CHECK (
        status IN ('AC', 'WA', 'RE', 'CE', 'TLE', 'MLE', 'IE')
    );
...
CREATE INDEX IF NOT EXISTS idx_judge_processes_submission_id ON public.judge_processes(submission_id);

CREATE INDEX IF NOT EXISTS idx_judge_processes_status ON public.judge_processes(status);

CREATE INDEX IF NOT EXISTS idx_judge_processes_created_at ON public.judge_processes(created_at DESC);

-- Judge Case Results indexes
CREATE INDEX IF NOT EXISTS idx_judge_case_results_process_id ON public.judge_case_results(judge_process_id);

CREATE INDEX IF NOT EXISTS idx_judge_case_results_case_id ON public.judge_case_results(judge_case_id);

CREATE INDEX IF NOT EXISTS idx_judge_case_results_status ON public.judge_case_results(status);

-- Judge Case Result Metadata indexes
CREATE INDEX IF NOT EXISTS idx_judge_case_result_metadata_result_id ON public.judge_case_result_metadata(judge_case_result_id);
...
ALTER TABLE
    public.submissions ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.judge_processes ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.judge_case_results ENABLE ROW LEVEL SECURITY;

ALTER TABLE
    public.judge_case_result_metadata ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "judge_system_can_update_processes" ON public.judge_processes FOR
UPDATE
    USING (
...
CREATE POLICY "judge_system_can_insert_results" ON public.judge_case_results FOR
INSERT
    WITH CHECK (
        -- TODO: 実際のジャッジサービスアカウントのIDに置き換える
        true -- 一時的に全て許可、後でサービスアカウント制限を追加
...
-- =====================================================
CREATE TRIGGER handle_updated_at_submissions BEFORE
UPDATE
    ON public.submissions FOR EACH ROW EXECUTE PROCEDURE moddatetime (updated_at);

CREATE TRIGGER handle_updated_at_judge_processes BEFORE
UPDATE
    ON public.judge_processes FOR EACH ROW EXECUTE PROCEDURE moddatetime (updated_at);

CREATE TRIGGER handle_updated_at_judge_case_results BEFORE
UPDATE
    ON public.judge_case_results FOR EACH ROW EXECUTE PROCEDURE moddatetime (updated_at);

CREATE TRIGGER handle_updated_at_judge_case_result_metadata BEFORE
UPDATE
    ON public.judge_case_result_metadata FOR EACH ROW EXECUTE PROCEDURE moddatetime (updated_at);
...
RETURNS TRIGGER AS $$ 
BEGIN -- Update submission count
INSERT INTO
    public.user_problem_status (
        user_id,
        problem_id,
        submission_count,
        last_submission_id
...
VALUES
    (NEW.user_id, NEW.problem_id, 1, NEW.id) ON CONFLICT (user_id, problem_id) DO
UPDATE
SET
...
AFTER
INSERT
    ON public.submissions FOR EACH ROW EXECUTE FUNCTION update_user_problem_status_on_submission();

-- Trigger to update user stats and problem status when judge process completes
CREATE
OR REPLACE FUNCTION update_user_stats_on_judge_completion() RETURNS TRIGGER AS $$ DECLARE submission_record public.submissions % ROWTYPE;

is_solved BOOLEAN := FALSE;

BEGIN -- Only process when status changes to 'succeeded'
IF NEW.status = 'succeeded'
AND (
    OLD.status IS NULL
    OR OLD.status != 'succeeded'
) THEN -- Get submission info
SELECT
    * INTO submission_record
FROM
    public.submissions
WHERE
    id = NEW.submission_id;

-- Check if this submission solved the problem (assuming score >= 100.0 means solved)
IF NEW.result = 'AC'
OR (
    submission_record.score IS NOT NULL
    AND submission_record.score >= 100.0
) THEN is_solved := TRUE;
...
SET
    ...
    best_score = GREATEST(
        ...
        COALESCE(submission_record.score, 0)
    ),
    updated_at = NOW()
WHERE
...
INSERT INTO
    public.user_stats (user_id, problems_solved, submissions_count)
VALUES
    (
        submission_record.user_id,
        CASE
            WHEN is_solved THEN 1
            ELSE 0
        END,
        1
...
AFTER
UPDATE
    ON public.judge_processes FOR EACH ROW EXECUTE FUNCTION update_user_stats_on_judge_completion();
