...
from ..const import CACHE_MAX_SIZE, CACHE_TTL_SECONDS
...
class CacheManager:
    ...
    async def get(self, namespace: str, key: str, **kwargs) -> Any | None:
        """値を取得"""
        if not self.backend:
            return None
    ...
    async def set(self, namespace: str, key: str, value: Any, ttl: int | None = None, **kwargs) -> bool:
        """値を設定"""
        if not self.backend:
            return False
...
# デコレータ関数
def cached(namespace: str, ttl: int | None = None, key_func: callable | None = None):
    """キャッシュデコレータ"""
