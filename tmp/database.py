...
class BaseRepository(Generic[T]):
    ...
    async def find_by_id(self, id: str | uuid.UUID) -> T | None:
        """IDでエンティティを取得"""
        data = await self.supabase_client.select(self.table_name, filters={"id": str(id)})
    ...
    async def delete(self, id: str | uuid.UUID) -> bool:
        """エンティティを削除"""
        try:
            deleted_data = await self.supabase_client.delete(self.table_name, filters={"id": str(id)})
