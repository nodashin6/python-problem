...
class BaseSupabaseRepository(ABC):
    ...
    async def _handle_error(self, error: Exception, operation: str) -> None:
        ...
        logger.error(f"Repository error in {operation}: {error!s}")
        raise
...
class JudgeCaseRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, judge_case_id: UUID) -> JudgeCase | None:
        pass
...
class SubmissionRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def get_by_problem_id(self, problem_id: UUID, limit: int = 10) -> list[Submission]:
        pass
...
class JudgeProcessRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, judge_process_id: UUID) -> JudgeProcess | None:
        pass
...
class JudgeCaseResultRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def get_by_id(self, result_id: UUID) -> JudgeCaseResult | None:
        pass

    @abstractmethod
    async def get_by_judge_process_id(self, judge_process_id: UUID) -> list[JudgeCaseResult]:
        pass
...
class UserProblemStatusRepositoryInterface(ABC):
    ...
    @abstractmethod
    async def get_by_user_and_problem(self, user_id: UUID, problem_id: UUID) -> UserProblemStatus | None:
        pass
...
class SupabaseBookRepository(BaseSupabaseRepository, BookRepositoryInterface):
    ...
    async def get_by_id(self, book_id: UUID) -> Book | None:
        try:
            result = await self.supabase.table("books").select("*").eq("id", str(book_id)).execute()
    ...
    async def get_published(self) -> list[Book]:
        try:
            result = await self.supabase.table("books").select("*").eq("is_published", True).execute()
    ...
    async def delete(self, book_id: UUID) -> bool:
        try:
            result = await self.supabase.table("books").delete().eq("id", str(book_id)).execute()
...
class SupabaseProblemRepository(BaseSupabaseRepository, ProblemRepositoryInterface):
    ...
    async def create(self, problem: Problem) -> Problem:
        try:
            result = (
                await self.supabase.table("problem_headers")
                .insert(
                    {
                        "title": problem.title,
    ...
    async def get_by_id(self, problem_id: UUID) -> Problem | None:
        try:
            result = (
                await self.supabase.table("problem_headers").select("*").eq("id", str(problem_id)).execute()
            )
    ...
    async def get_by_book_id(self, book_id: UUID) -> list[Problem]:
        try:
            ...
            return [Problem(**problem) for problem in result.data] if result.data else []
    ...
    async def delete(self, problem_id: UUID) -> bool:
        try:
            result = await self.supabase.table("problems").delete().eq("id", str(problem_id)).execute()
            return len(result.data) > 0 if result.data else False
...
class SupabaseJudgeCaseRepository(BaseSupabaseRepository, JudgeCaseRepositoryInterface):
    ...
    async def get_by_id(self, judge_case_id: UUID) -> JudgeCase | None:
        try:
            result = (
                await self.supabase.table("judge_cases").select("*").eq("id", str(judge_case_id)).execute()
            )
    ...
    async def delete(self, judge_case_id: UUID) -> bool:
        try:
            result = (
                await self.supabase.table("judge_cases").delete().eq("id", str(judge_case_id)).execute()
            )
            return len(result.data) > 0 if result.data else False
...
class SupabaseSubmissionRepository(BaseSupabaseRepository, SubmissionRepositoryInterface):
    ...
    async def get_by_id(self, submission_id: UUID) -> Submission | None:
        try:
            result = (
                await self.supabase.table("submissions").select("*").eq("id", str(submission_id)).execute()
            )
    ...
    async def get_by_user_id(self, user_id: UUID, limit: int = 10) -> list[Submission]:
        try:
            ...
            return [Submission(**submission) for submission in result.data] if result.data else []
    ...
    async def get_by_user_and_problem(
        ...
        try:
            ...
            await self._handle_error(e, f"get submissions by user {user_id} and problem {problem_id}")
    ...
    async def delete(self, submission_id: UUID) -> bool:
        try:
            result = (
                await self.supabase.table("submissions").delete().eq("id", str(submission_id)).execute()
            )
            return len(result.data) > 0 if result.data else False
...
class SupabaseJudgeProcessRepository(BaseSupabaseRepository, JudgeProcessRepositoryInterface):
    ...
    async def get_by_submission_id(self, submission_id: UUID) -> list[JudgeProcess]:
        try:
            ...
            return [JudgeProcess(**process) for process in result.data] if result.data else []

        except Exception as e:
            await self._handle_error(e, f"get judge processes by submission {submission_id}")
...
class SupabaseJudgeCaseResultRepository(BaseSupabaseRepository, JudgeCaseResultRepositoryInterface):
        ...
        try:
            ...
            if result.data:
                ...
                if judge_case_result.metadata:
                    await self._create_metadata(case_result.id, judge_case_result.metadata)
    ...
    async def _create_metadata(self, result_id: UUID, metadata: JudgeCaseResultMetadata) -> None:
        """Create metadata for judge case result"""
        await (
            self.supabase.table("judge_case_result_metadata")
            .insert(
                {
        ...
        try:
            ...
            if result.data:
                ...
                if data.get("judge_case_result_metadata"):
                    metadata = JudgeCaseResultMetadata(**data["judge_case_result_metadata"][0])
    ...
    async def get_by_judge_process_id(self, judge_process_id: UUID) -> list[JudgeCaseResult]:
        try:
            ...
            await self._handle_error(e, f"get judge case results by process {judge_process_id}")
    ...
    async def update(self, judge_case_result: JudgeCaseResult) -> JudgeCaseResult:
        try:
            ...
            if result.data:
                ...
                if judge_case_result.metadata:
                    await self._update_metadata(judge_case_result.id, judge_case_result.metadata)

                return JudgeCaseResult(**result.data[0])
            raise Exception("Failed to update judge case result")

        except Exception as e:
            await self._handle_error(e, f"update judge case result {judge_case_result.id}")

    async def _update_metadata(self, result_id: UUID, metadata: JudgeCaseResultMetadata) -> None:
    ...
    async def delete(self, result_id: UUID) -> bool:
        try:
            result = (
                await self.supabase.table("judge_case_results").delete().eq("id", str(result_id)).execute()
            )
            return len(result.data) > 0 if result.data else False
...
class SupabaseUserProblemStatusRepository(BaseSupabaseRepository, UserProblemStatusRepositoryInterface):
    ...
    async def get_by_user_and_problem(self, user_id: UUID, problem_id: UUID) -> UserProblemStatus | None:
        try:
            ...
            await self._handle_error(e, f"get user problem status for user {user_id} problem {problem_id}")
    ...
    async def get_by_user_id(self, user_id: UUID) -> list[UserProblemStatus]:
        try:
            ...
            return [UserProblemStatus(**status) for status in result.data] if result.data else []
    ...
    async def update(self, status: UserProblemStatus) -> UserProblemStatus:
        try:
            ...
            await self._handle_error(e, f"update user problem status {status.user_id}/{status.problem_id}")
    ...
    async def upsert(self, status: UserProblemStatus) -> UserProblemStatus:
        try:
            ...
            await self._handle_error(e, f"upsert user problem status {status.user_id}/{status.problem_id}")
