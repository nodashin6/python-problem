...
from src.domain.services.judge_service import JudgeResultStatus, JudgeService
...
def error_handler(func):
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            logger.warning(f"リソースが見つかりません: {e!s}")
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"リソースが見つかりません: {e!s}",
            )
        except Exception as e:
            # その他の予期しないエラーは500を返す
            logger.error(f"予期しないエラーが発生しました: {e!s}")
            logger.debug(traceback.format_exc())
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"サーバーエラーが発生しました: {e!s}",
            )
