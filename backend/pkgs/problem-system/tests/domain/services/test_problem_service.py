...
class TestProblemDomainService:
    ...
    @pytest.fixture
    def problem_service(self, mock_problem_repo, mock_judge_case_repo, mock_content_repo, mock_event_bus):
    ...
    @pytest.mark.asyncio
    async def test_create_problem_success(self, problem_service, mock_problem_repo, mock_event_bus):
        """問題作成成功のテスト"""
        # モックの設定
    ...
    @pytest.mark.asyncio
    async def test_create_problem_duplicate_title(self, problem_service, mock_problem_repo):
        """問題作成時のタイトル重複エラーのテスト"""
        # モックの設定 - 既存の問題を返す
    ...
    @pytest.mark.asyncio
    async def test_add_sample_case_success(self, problem_service, mock_judge_case_repo, mock_event_bus):
        """サンプルケース追加成功のテスト"""
        problem_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_add_hidden_case_success(self, problem_service, mock_judge_case_repo, mock_event_bus):
        """隠しケース追加成功のテスト"""
        problem_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_get_problem_with_cases(self, problem_service, mock_problem_repo, mock_judge_case_repo):
        """問題とケースの取得テスト"""
        problem_id = uuid4()
        ...
        # テスト実行
        result_problem, result_cases = await problem_service.get_problem_with_cases(problem_id)
    ...
    @pytest.mark.asyncio
    async def test_publish_problem_success(self, problem_service, mock_problem_repo, mock_event_bus):
        """問題公開成功のテスト"""
        problem_id = uuid4()
        problem = Problem(
            id=problem_id,
    ...
    @pytest.mark.asyncio
    async def test_delete_problem_success(self, problem_service, mock_problem_repo, mock_judge_case_repo):
        """問題削除成功のテスト"""
        problem_id = uuid4()
        problem = Problem(
            id=problem_id,
