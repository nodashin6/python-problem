...
async def get_problem_controller() -> ProblemController:
    """問題コントローラーを取得"""
    return ProblemController(container.problem_service(), container.user_status_service())
...
@core_router.get("/problems/{problem_id}/judge-cases", response_model=List[JudgeCaseResponse])
...
@core_router.get("/users/{user_id}/status/{problem_id}", response_model=UserStatusResponse)
...
@core_router.put("/users/{user_id}/status/{problem_id}", response_model=UserStatusResponse)
...
@core_router.post("/auth/register", response_model=AuthResponse, status_code=status.HTTP_201_CREATED)
